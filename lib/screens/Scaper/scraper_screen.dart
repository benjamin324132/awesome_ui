...
class _ScraperScreenState extends State<ScraperScreen> {
  ...
  getData() async {
    ...
    for (int i = 0; i < data.length; i++) {
      var user = data[i].querySelectorAll(
          "div.shot-details-container > div.user-information > a.url > span.display-name")[0];
      var img =
          data[i].querySelectorAll("div.shot-thumbnail-base > figure > img")[0];
      //print("Index: $i elemtent: ${data[i].children[0].toString()}");
      print(user.text.toString().trim());
      print(img.attributes['src']);
      items.add(
          {"title": user.text.toString().trim(), "img": img.attributes['src']});
